...
const setCookies=async(res:Response,userId:string,version:number,refreshToken:string)=>{const settings=await getSettings();res.cookie("access_token",signAccessToken({sub:userId,ver:version}),{...cookieBase,maxAge:settings.security.sessionTimeoutMinutes*60*1000});res.cookie("refresh_token",refreshToken,{...cookieBase,maxAge:30*86400000});};
export async function login(req:Request,res:Response){const {identifier,password}=req.body;if(typeof identifier!=="string"||typeof password!=="string")throw Object.assign(new Error("Validation failed"),{statusCode:422,code:"VALIDATION_ERROR"});const result=await service.login(identifier,password,{ip:req.ip,userAgent:req.get("user-agent")});await setCookies(res,result.user.id,result.version,result.refreshToken);res.json({user:result.user});}
export async function refresh(req:Request,res:Response){const token=readCookies(req.headers.cookie).refresh_token;if(!token)throw Object.assign(new Error("Refresh token required"),{statusCode:401,code:"UNAUTHENTICATED"});const result=await service.refresh(token);await setCookies(res,result.user.id,result.version,result.refreshToken);res.json({user:result.user});}
...
export async function updateProfile(req:AuthRequest,res:Response){res.json({user:await service.updateProfile(req.auth!.id,req.body)});}
